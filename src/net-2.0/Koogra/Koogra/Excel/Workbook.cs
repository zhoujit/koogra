...
namespace Net.SourceForge.Koogra.Excel
{
    ...
    public class Workbook : IWorkbook
    {
        ...
        public System.Data.DataSet AsDataSet(bool firstRowAsColumnNames) {
            DataSet set = new DataSet();
            foreach (var sheet in _sheets) {
                DataTable table = set.Tables.Add(sheet.Name);
                uint maxCol = sheet.Rows.InternalValues.Max(x => x.Cells.MaxCol);
                uint minCol = sheet.Rows.InternalValues.Min(x => x.Cells.MinCol);
                uint startRow = sheet.Rows.MinRow;

                if (firstRowAsColumnNames) {
                    Row firstRow = sheet.Rows[0];
                    uint col;
                    for (col = minCol; col <= maxCol && col <= firstRow.Cells.MaxCol; col++) {
                        table.Columns.Add(firstRow.Cells[col].Value.ToString(), typeof(object));                        
                    }
                ...
                for (uint i = startRow; i <= sheet.Rows.MaxRow; i++) {
                    Row row = sheet.Rows[i];
                    DataRow newRow = table.NewRow();
                    for (uint j = row.Cells.MinCol; j <= row.Cells.MaxCol; j++) {
                        if(row.Cells[j] != null)
                            newRow[(int)j] = row.Cells[j].Value;
                    }
